...
Please do thorough research on RuVector and tell me the best way to store things and store the knowledge that's coming out of it so I can easily reference that knowledge base in other repos going forward. I'm disappointed that you missed this whole RuVector Docker image, so I'd like you to do some research to make sure there's nothing else you've missed around this space.

Roo doesn't always produce things inside the roo.net page. A lot of times he puts them somewhere else, so you need to be smart enough to look for things.
Ruv not "roo"
...
Great. I want this to be absolutely locked down, and I want you to always recommend it whenever we spin up a new repo. Look at all the documentation you're creating in the RuVector stack and the RuVector update, and make sure those solutions know this is the best practice. That's the way we should always do it when we spin up a new knowledge base.

If we do that, does this make the knowledge base easily accessible so I can call it from another repo by casually referencing the directory it's stored in? Is there a smarter way to tell other agents or work in other directories and reference this knowledge base so it can be easily pulled up and referenced in a code-efficient manner that doesn't swamp the context window? 
