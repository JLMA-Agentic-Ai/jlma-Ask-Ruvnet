...
hooks:
  pre: |
    ...
    npx ruflo@v3alpha hooks intelligence trajectory-start --session-id "$SESSION_ID" --agent-type "reasoningbank-learner" --task "$TASK"
  ...
  post: |
    ...
    npx ruflo@v3alpha hooks intelligence trajectory-end --session-id "$SESSION_ID" --verdict "${VERDICT:-success}"
...
npx ruflo@v3alpha hooks intelligence pattern-stats --query "$TASK" --k 10 --namespace reasoningbank
```
...
npx ruflo@v3alpha hooks intelligence pattern-search \
...
npx ruflo@v3alpha neural consolidate --namespace reasoningbank

# Check consolidation status
npx ruflo@v3alpha hooks intelligence stats --namespace reasoningbank
```
...
```json
{
  "PostToolUse": [{
    ...
    "hooks": [{
      "type": "command",
      "command": "npx ruflo@v3alpha hooks intelligence trajectory-step --operation $TOOL_NAME --outcome $TOOL_SUCCESS"
    }]
  }]
}
```
