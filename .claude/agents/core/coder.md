...
hooks:
  pre: |
    ...
    SIMILAR_PATTERNS=$(npx ruflo@v3alpha memory search --query "$TASK" --limit 5 --min-score 0.8 --use-hnsw)
    ...
    FAILURES=$(npx ruflo@v3alpha memory search --query "$TASK failures" --limit 3 --failures-only)
    if [ -n "$FAILURES" ]; then
  ...
  post: |
    ...
    npx ruflo@v3alpha hooks post-task --task-id "coder-$(date +%s)" --success "$SUCCESS"
    ...
    npx ruflo@v3alpha hooks worker dispatch --trigger consolidate
---
