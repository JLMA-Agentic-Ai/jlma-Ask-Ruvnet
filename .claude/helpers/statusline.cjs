...
// Read JSON input from Claude Code stdin (authoritative model source)
let stdinInput = null;
try {
  stdinInput = JSON.parse(fs.readFileSync('/dev/stdin', 'utf-8'));
} catch (e) {
  // No stdin or invalid JSON — will fall back to other sources
}

// Parse model ID to human-readable name
function parseModelName(modelId) {
  if (!modelId) return null;
  if (modelId.includes('opus-4-6') || modelId.includes('opus-4.6')) return 'Opus 4.6';
  if (modelId.includes('opus-4-5') || modelId.includes('opus-4.5')) return 'Opus 4.5';
  if (modelId.includes('sonnet-4-6') || modelId.includes('sonnet-4.6')) return 'Sonnet 4.6';
...
function getUserInfo() {
  ...
  // Priority 1: Claude Code JSON stdin (authoritative — tells us the CURRENT model)
  if (stdinInput && stdinInput.model) {
    // Prefer parsed ID (includes version like "Opus 4.6") over display_name (just "Opus")
    modelName = parseModelName(stdinInput.model.id) || stdinInput.model.display_name || null;
  }

  // Priority 2: ~/.claude.json lastModelUsage (may be stale but better than nothing)
  if (!modelName) {
    try {
      ...
      if (fs.existsSync(claudeConfigPath)) {
        ...
        if (lastModelUsage) {
          ...
          if (modelIds.length > 0) {
            ...
            modelName = parseModelName(modelId);
          }
        }
      }
    } catch (e) {
  ...
  // Priority 3: project settings.json model field
  if (!modelName) {
    try {
      const settingsPath = path.join(process.cwd(), '.claude', 'settings.json');
      if (fs.existsSync(settingsPath)) {
        const settings = JSON.parse(fs.readFileSync(settingsPath, 'utf-8'));
        if (settings.claudeFlow && settings.claudeFlow.modelPreferences) {
          modelName = parseModelName(settings.claudeFlow.modelPreferences.default);
        }
        if (!modelName && settings.model) {
          modelName = parseModelName(settings.model);
        }
      }
    } catch (e) {
      // Keep null
