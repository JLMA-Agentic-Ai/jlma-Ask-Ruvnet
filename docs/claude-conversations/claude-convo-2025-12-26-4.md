...
No I don't use the HeroForge AI anymore so please just use the SI Kerr at Gmail and make sure everything is set up for that one. If you need to know that that's authenticated go in and use the Playwright tool and add any other capabilities you need to it to have full imagen, mapping, google gemini 3+ etc  access 
...
Great can you store all of this information in a global user level memory so that you know exactly where all this information is at a global level and you know how to access it and update it if we ever need to reauthenticate 
...
When I restart can I do this using slash continue or do I have to start a brand new Cloud Code session ?I'd really like to be able to use the dash continue so it remembers the context of where we're at will that work 
