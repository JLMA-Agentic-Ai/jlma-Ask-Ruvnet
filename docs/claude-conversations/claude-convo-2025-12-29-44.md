...
<agent-notification>
<agent-id>a227fdb</agent-id>
<output-file>/tmp/claude/-Users-stuartkerr-Code-Ask-Ruvnet-Ask-Ruvnet/tasks/a227fdb.output</output-file>
<status>completed</status>
<summary>Agent "Extract working examples" completed.</summary>
...
Do research on RuvNet-Postgres and what its strategy is. Is that the best way we should be storing all of this? If so, please implement it. 
...
keep pushing.   Make sure you're managing the context window effectively. It keeps stopping and saying the context window is too low. It won't even allow you to compact, and then I have to stop and reverse the whole process. The context window you're showing in the status window doesn't seem to be displaying correctly.

There are big discrepancies between that and what it tells me I actually have.
