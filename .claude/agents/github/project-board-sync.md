...
npx ruflo@v3alpha github board-init \
...
npx ruflo@v3alpha github board-sync \
  --map-status '{
    "todo": "To Do",
...
npx ruflo@v3alpha github board-realtime \
...
npx ruflo@v3alpha github board-auto-assign \
...
npx ruflo@v3alpha github board-progress \
...
npx ruflo@v3alpha github board-smart-move \
  --rules '{
...
# Process with swarm
npx ruflo@v3alpha github board-import-issues \
  --issues "$ISSUES" \
...
npx ruflo@v3alpha github board-bulk \
...
npx ruflo@v3alpha github board-template \
...
npx ruflo@v3alpha github multi-board-sync \
...
npx ruflo@v3alpha github cross-org-sync \
...
npx ruflo@v3alpha github board-integrate \
  --tool "jira" \
...
npx ruflo@v3alpha github board-analytics \
...
```bash
# Generate reports
npx ruflo@v3alpha github board-report \
  --type "sprint-summary" \
  --format "markdown" \
...
npx ruflo@v3alpha github sprint-manage \
  --sprint "Sprint 23" \
  --auto-populate \
  --capacity-planning \
  --track-velocity
...
npx ruflo@v3alpha github milestone-track \
...
npx ruflo@v3alpha github release-plan-board \
  --analyze-velocity \
  --estimate-completion \
  --identify-risks \
  --optimize-scope
...
npx ruflo@v3alpha github board-distribute \
...
npx ruflo@v3alpha github standup-report \
  --team "frontend" \
...
npx ruflo@v3alpha github review-coordinate \
  --board "Code Review" \
  --assign-reviewers \
  --track-feedback \
  --ensure-coverage
...
npx ruflo@v3alpha github board-diagnose \
...
npx ruflo@v3alpha github board-optimize \
  --analyze-size \
  --archive-completed \
  --index-fields \
  --cache-views
...
```bash
# Recover board data
npx ruflo@v3alpha github board-recover \
...
```bash
# Setup agile board
npx ruflo@v3alpha github agile-board \
  --methodology "scrum" \
  --sprint-length "2w" \
...
```bash
# Setup kanban board
npx ruflo@v3alpha github kanban-board \
  --wip-limits '{
    "In Progress": 5,
    "Review": 3
  }' \
...
npx ruflo@v3alpha github research-board \
...
npx ruflo@v3alpha github board-kpis \
  --metrics '[
    "average-cycle-time",
...
npx ruflo@v3alpha github team-metrics \
  --board "Development" \
  --per-member \
