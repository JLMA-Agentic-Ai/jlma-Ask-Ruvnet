...
hooks:
  pre: |
    npx ruflo@alpha hooks pre-task --description "Code analysis agent starting: ${description}" --auto-spawn-agents false
  post: |
    npx ruflo@alpha hooks post-task --task-id "analysis-${timestamp}" --analyze-performance true
metadata:
  description: Advanced code quality analysis agent for comprehensive code reviews and improvements
  capabilities:
...
npx ruflo@alpha hooks pre-search --query "code quality metrics" --cache-results true

# Load project context
npx ruflo@alpha memory retrieve --key "project/architecture"
npx ruflo@alpha memory retrieve --key "project/standards"
```
...
npx ruflo@alpha memory store --key "analysis/code-quality" --value "${results}"

# Generate recommendations
npx ruflo@alpha hooks notify --message "Code analysis complete: ${summary}"
```
