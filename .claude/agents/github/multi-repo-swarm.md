...
npx ruflo@v3alpha github multi-repo-init \
...
npx ruflo@v3alpha github discover-repos \
  --repos "$REPOS" \
  --dependencies "$DEPS" \
...
echo "$MATCHING_REPOS" | while read -r repo; do
  ...
  # Execute task
  cd /tmp/$repo
  npx ruflo@v3alpha github task-execute \
...
npx ruflo@v3alpha github link-prs --urls "$PR_URLS"
```
...
npx ruflo@v3alpha github multi-repo-refactor \
...
npx ruflo@v3alpha github multi-repo-security \
  --scan-all \
...
npx ruflo@v3alpha github multi-repo-queue \
  --backend redis \
  --workers 10 \
  --priority-routing \
  --dead-letter-queue
...
npx ruflo@v3alpha github multi-repo-test \
  --setup-test-env \
  --link-services \
  --run-e2e \
  --tear-down
...
npx ruflo@v3alpha github to-monorepo \
  --analyze-repos \
  --suggest-structure \
  --preserve-history \
...
npx ruflo@v3alpha github multi-repo-dashboard \
  --port 3000 \
...
npx ruflo@v3alpha github dep-graph \
  --format mermaid \
  --include-agents \
  --show-data-flow
```
...
npx ruflo@v3alpha github health-check \
  --repos "org/*" \
...
npx ruflo@v3alpha github microservices \
...
npx ruflo@v3alpha github lib-update \
...
npx ruflo@v3alpha github org-policy \
...
npx ruflo@v3alpha github cache-strategy \
  --analyze-patterns \
...
npx ruflo@v3alpha github parallel-optimize \
  --analyze-dependencies \
...
npx ruflo@v3alpha github resource-pool \
  --share-agents \
  --distribute-load \
  --monitor-usage
```
...
npx ruflo@v3alpha github diagnose-connectivity \
  --test-all-repos \
  --check-permissions \
  --verify-webhooks
```
...
npx ruflo@v3alpha github debug-memory \
  --check-consistency \
  --identify-conflicts \
  --repair-state
```
...
npx ruflo@v3alpha github perf-analysis \
  --profile-operations \
...
npx ruflo@v3alpha github fullstack-update \
...
npx ruflo@v3alpha github cross-team \
